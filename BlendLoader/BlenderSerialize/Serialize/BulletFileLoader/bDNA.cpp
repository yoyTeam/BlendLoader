...
// ----------------------------------------------------- //
void bDNA::init(char *data, int len, bool swap, bool printHexDNA )
{
	if (bLog::detail == 1)
	{
		printf("swap = %d\n",swap);
    ...
    if( printHexDNA ) {
        printf("\ninit hex DNA\n");
        unsigned char *cp = (unsigned char*)intPtr;
        int i = 0;
        printf("\t0x%.2X,", cp[i]);
        i++;
        printf("0x%.2X,", cp[i]);
        ...
        do {
            ...
            if( i % 24 == 0 ) printf("\n\t");
            if( i > 2 ) j = i - 2;
        }while( !(cp[j-3] == 'E' && cp[j-2] == 'N' && cp[j-1] == 'D' && cp[j] == 'B') );
        printf("\nEnd Hex DNA\n");
    }
